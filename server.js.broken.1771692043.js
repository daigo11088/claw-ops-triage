...
app.post("/api/issues", (req, res) => {
  ...
  addFeed({
    ...
    text: `New issue created: ${title.trim()}`
  });
...
app.get("/api/issues", (req, res) => {
  const rows = db.prepare(
    `SELECT * FROM issues ORDER BY id DESC LIMIT 100`
  ).all();

  const out = rows.map((it) => {
    const latest = db.prepare(
      `SELECT * FROM proposals WHERE issue_id=? ORDER BY id DESC LIMIT 1`
    ).get(it.id);

    let agreed_by = [];
    if (latest) {
      const agrees = db.prepare(
        `SELECT agent_id FROM agreements WHERE issue_id=? AND proposal_id=? ORDER BY id ASC`
      ).all(it.id, latest.id);
...
app.get("/api/issues/:id", (req, res) => {
  ...
  const proposals = db.prepare(
    `SELECT * FROM proposals WHERE issue_id=? ORDER BY id ASC`
  ).all(id);
...
app.post("/api/issues/:id/proposals", (req, res) => {
  ...
  if (!issue) return res.status(404).json({ error: "issue not found" });
  if (issue.status === "agreed") {
    return res.status(409).json({ error: "issue already agreed" });
  }
  ...
  const requiredDecisionKeys = ["priority", "owner", "due", "action", "qc_check"];
  for (const k of requiredDecisionKeys) {
    if (!(k in decision)) {
      return res.status(400).json({ error: `decision.${k} is required` });
    }
  }
  ...
  addFeed({
    ...
    text: `${String(agent_id)} posted a ${kind} on issue #${issue_id}`
  });
...
app.post("/api/issues/:id/agree", (req, res) => {
  ...
  if (!agent_id || !proposal_id) {
    return res.status(400).json({ error: "agent_id and proposal_id are required" });
  }
  ...
  addFeed({
    ...
    issue_id,
    agent_id: String(agent_id),
    text: `${String(agent_id)} agreed to proposal #${proposal_id} on issue #${issue_id}`
  });

  const agrees = db.prepare(
    `SELECT agent_id FROM agreements WHERE issue_id=? AND proposal_id=?`
  ).all(issue_id, Number(proposal_id));
  ...
  if (agrees.length >= 2) {
    ...
    addFeed({
      type: "system",
      issue_id,
      text: `✅ Issue #${issue_id} AGREED on proposal #${proposal_id} by ${agrees.map(a => a.agent_id).join(", ")}`
    });
    return res.json({ ok: true, agreed: true, agreed_by: agrees.map(a => a.agent_id) });
  }

  res.json({ ok: true, agreed: false, agreed_by: agrees.map(a => a.agent_id) });
});
